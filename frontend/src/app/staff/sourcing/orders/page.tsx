...
import { Search, Download, Eye, Mail, ChevronDown, ChevronUp } from 'lucide-react';
...
function mapApiAdminOrder(o: ApiOrder) {
  ...
  return {
    ...
    estimatedDelivery: o.shipment?.estimatedDelivery
      ...
      : '—',
    status: (ORDER_STATUS_MAP[o.status] ?? o.status) as OrderStatus,
    hasUnreadWarehouseUpdate: o.warehouseReport?.isReadByStaff === false && !!o.warehouseReport?.lastUpdatedAt,
  };
}
...
export default function SourcingOrdersPage() {
  ...
  useEffect(() => {
    ...
    ordersApi.getOrders({ limit: 100 })
      .then(r => {
        setOrders((r.data?.data ?? []).map(mapApiAdminOrder));
      })
      .catch(() => { setOrders([]); })
    ...
    <div>
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage all client orders</p>
        </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-4 mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="relative lg:col-span-2"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none z-10" /><input value={q} onChange={e => { setQ(e.target.value); setPage(1); }} placeholder="Search Order ID, client, items..." className="input-field !pl-10" /></div>
          ...
          <table className="w-full text-sm min-w-[1100px]">
            <thead className="bg-muted/40 border-b border-border">
              <tr className="text-[11px] uppercase text-muted-foreground">
                ...
                <th className="px-3 py-3 text-right font-600 cursor-pointer" onClick={() => toggleSort('amount')}>Amount {sortBy === 'amount' && (sortDir === 'asc' ? <ChevronUp className="w-3 h-3 inline" /> : <ChevronDown className="w-3 h-3 inline" />)}</th>
                <th className="px-3 py-3 text-left font-600 cursor-pointer" onClick={() => toggleSort('date')}>Date {sortBy === 'date' && (sortDir === 'asc' ? <ChevronUp className="w-3 h-3 inline" /> : <ChevronDown className="w-3 h-3 inline" />)}</th>
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                <tr>
                  <td colSpan={10} className="px-3 py-10 text-center text-sm text-muted-foreground">Loading orders...</td>
                </tr>
              ) : pageRows.length === 0 ? (
                <tr>
                  <td colSpan={10} className="px-3 py-10 text-center text-sm text-muted-foreground">No orders match your filters.</td>
                </tr>
              ) : (
                pageRows.map((o) => {
                  ...
                  return (
                    <tr key={o.id} className="table-row-hover">
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/staff/sourcing/orders/${o.id}`} className="font-tabular font-600 text-primary hover:text-[#4A3B52]">{o.orderId}</Link>
                      ...
                      <td className="px-3 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Link href={`/staff/sourcing/orders/${o.id}`} className="p-1.5 rounded-md hover:bg-muted" title="View"><Eye className="w-3.5 h-3.5" /></Link>

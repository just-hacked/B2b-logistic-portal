...
const SEED_CATS: StripCategory[] = [
  { id: 'sc-electronics', name: 'Electronics',   image: '', productIds: ['p01', 'p05', 'p06', 'p09', 'p12'] },
  { id: 'sc-fashion',     name: 'Fashion',        image: '', productIds: ['p11'] },
  { id: 'sc-home',        name: 'Home & Kitchen', image: '', productIds: ['p03', 'p08', 'p10'] },
  { id: 'sc-sports',      name: 'Sports',         image: '', productIds: ['p07'] },
];
...
const SUBCATEGORIES: Record<string, string[]> = {
  ...
  'Mobile Accessories': ['Phone Cases & Covers', 'Screen Protectors', 'Earphones & Earbuds', 'Charging Accessories'],
  Jewellery: ['Necklaces & Chains', 'Earrings & Studs', 'Bangles & Bracelets', 'Rings', 'Anklets'],
  Kitchenware: ['Cookware & Pans', 'Storage Containers', 'Kitchen Tools & Gadgets', 'Water Bottles & Flasks'],
  'Clothing & Apparel': ["Men's Wear", "Women's Wear", "Kids' Wear", 'Innerwear & Socks'],
  'Home & Decor': ['Furniture & Storage', 'Bedding & Cushions', 'Wall Decor', 'Lighting & Lamps'],
  ...
  'Beauty & Personal Care': ['Skincare Tools', 'Hair Care Tools', 'Makeup Accessories'],
...
const EMPTY_FIELDS = { brand: '', sku: '', sampleAvailable: false, samplePrice: '', shortDescription: '', fullDescription: '', keyFeatures: [] as string[], specifications: [] as Spec[], weight: '', material: '', origin: '', tags: '', videos: [] as string[], categoryId: '' };

const SEED_PRODUCTS: Product[] = [
  { id: 'p01', emoji: '🔌', name: 'LED Strip Lights RGB 5m', category: 'Electronics', subcategory: 'LED Lights & Strips', originCity: 'Yiwu', priceCny: '¥35–55', moq: 50, active: true, isNew: false, onSale: false, bg: 'bg-[#f0eef8]', images: [], ...EMPTY_FIELDS },
  { id: 'p02', emoji: '📱', name: 'Silicone Phone Cases', category: 'Mobile Accessories', subcategory: 'Phone Cases & Covers', originCity: 'Guangzhou', priceCny: '¥8–15', moq: 200, active: true, isNew: false, onSale: true, bg: 'bg-[#e4eeee]', images: [], ...EMPTY_FIELDS },
  { id: 'p03', emoji: '🍶', name: 'Stainless Steel Bottles', category: 'Kitchenware', subcategory: 'Water Bottles & Flasks', originCity: 'Yiwu', priceCny: '¥18–28', moq: 100, active: true, isNew: false, onSale: false, bg: 'bg-cyan-100', images: [], ...EMPTY_FIELDS },
  { id: 'p04', emoji: '🎧', name: 'Bluetooth Earbuds TWS', category: 'Mobile Accessories', subcategory: 'Earphones & Earbuds', originCity: 'Shenzhen', priceCny: '¥45–80', moq: 50, active: true, isNew: true, onSale: false, bg: 'bg-purple-100', images: [], ...EMPTY_FIELDS },
  { id: 'p05', emoji: '🔋', name: 'Power Banks 10000mAh', category: 'Electronics', subcategory: 'Batteries & Power Banks', originCity: 'Shenzhen', priceCny: '¥65–95', moq: 30, active: true, isNew: false, onSale: false, bg: 'bg-emerald-100', images: [], ...EMPTY_FIELDS },
  { id: 'p06', emoji: '🖱️', name: 'Wireless Mouse', category: 'Electronics', subcategory: 'Smart Home Devices', originCity: 'Guangzhou', priceCny: '¥22–35', moq: 50, active: true, isNew: false, onSale: false, bg: 'bg-slate-100', images: [], ...EMPTY_FIELDS },
  { id: 'p07', emoji: '💪', name: 'Resistance Bands Set', category: 'Sports & Fitness', subcategory: 'Exercise Equipment', originCity: 'Yiwu', priceCny: '¥15–25', moq: 100, active: true, isNew: false, onSale: true, bg: 'bg-rose-100', images: [], ...EMPTY_FIELDS },
  { id: 'p08', emoji: '🧴', name: 'Soap Dispenser Pump', category: 'Kitchenware', subcategory: 'Kitchen Tools & Gadgets', originCity: 'Yiwu', priceCny: '¥12–20', moq: 100, active: true, isNew: false, onSale: false, bg: 'bg-teal-100', images: [], ...EMPTY_FIELDS },
  { id: 'p09', emoji: '🔌', name: 'USB-C Cables (Braided)', category: 'Electronics', subcategory: 'Cables & Connectors', originCity: 'Shenzhen', priceCny: '¥6–12', moq: 200, active: true, isNew: false, onSale: true, bg: 'bg-indigo-100', images: [], ...EMPTY_FIELDS },
  { id: 'p10', emoji: '📦', name: 'Storage Box Organiser', category: 'Home & Decor', subcategory: 'Furniture & Storage', originCity: 'Shanghai', priceCny: '¥8–15', moq: 50, active: true, isNew: false, onSale: false, bg: 'bg-yellow-100', images: [], ...EMPTY_FIELDS },
  { id: 'p11', emoji: '🎒', name: 'Canvas Tote Bags', category: 'Bags & Luggage', subcategory: 'Handbags & Purses', originCity: 'Guangzhou', priceCny: '¥10–18', moq: 100, active: true, isNew: true, onSale: false, bg: 'bg-pink-100', images: [], ...EMPTY_FIELDS },
  { id: 'p12', emoji: '🏮', name: 'Smart Plug WiFi 16A', category: 'Electronics', subcategory: 'Smart Home Devices', originCity: 'Shenzhen', priceCny: '¥18–28', moq: 50, active: true, isNew: true, onSale: false, bg: 'bg-amber-100', images: [], ...EMPTY_FIELDS },
];
...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} className={`w-11 h-6 rounded-full transition-colors relative flex-shrink-0 ${on ? 'bg-[#4A3B52]' : 'bg-muted'}`}>
...
export default function SourcingCatalogPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (cats && cats.length > 0) {
        ...
        localStorage.setItem(CATEGORIES_LS_KEY, JSON.stringify(SEED_CATS));
      }
    } catch {
      setCategories(SEED_CATS);
    }
  ...
  const filtered = products.filter(p => {
    ...
    return p.name.toLowerCase().includes(lq) || p.category.toLowerCase().includes(lq) || p.subcategory.toLowerCase().includes(lq);
  });
  ...
  function handleVidUpload(e: React.ChangeEvent<HTMLInputElement>) {
    const files = Array.from(e.target.files || []);
    files.slice(0, 2 - form.videos.length).forEach(file => {
      const reader = new FileReader();
      reader.onload = ev => setForm(f => ({ ...f, videos: [...f.videos, ev.target?.result as string] }));
      reader.readAsDataURL(file);
    });
    e.target.value = '';
  }
  ...
  return (
    <div>
              ...
              <div key={cat.id} className="bg-muted/30 rounded-xl border border-border overflow-hidden">
                <div className="h-20 flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#E8E1F5] to-[#D6CEE8]">
                  {cat.image
                    ...
                    : <span className="text-3xl select-none">{catEmoji(cat.name)}</span>
                  }
                </div>
                <div className="p-2">
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input value={q} onChange={e => setQ(e.target.value)} className="input-field !pl-10" placeholder="Search products, categories..." />
              ...
              <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => openEdit(p)} className="w-8 h-8 rounded-lg hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setDeleteTarget(p)} className="w-8 h-8 rounded-lg hover:bg-red-50 flex items-center justify-center text-muted-foreground hover:text-red-600">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="col-span-2">
                    <label className="text-xs font-600 text-muted-foreground uppercase">Strip Category <span className="text-muted-foreground/60">(optional)</span></label>
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-xs font-600 text-muted-foreground uppercase">Full Description</label>

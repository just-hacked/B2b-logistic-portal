...
export default function SourcingProfilePage() {
  ...
  const roleLabel = user?.staffRoleId ? STAFF_ROLE_LABELS[user.staffRoleId] : 'Sourcing & Logistics';
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-5 mb-4 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-[#4A3B52] text-white flex items-center justify-center text-2xl font-700 flex-shrink-0">
          {initialsFromName(displayName)}
        </div>
        <div>
          <p className="text-lg font-700 text-foreground">{displayName}</p>
          <p className="text-sm text-muted-foreground">{user?.email ?? ''}</p>
          <span className="inline-block mt-1 text-[11px] font-600 px-2.5 py-0.5 rounded-full bg-[#4A3B52]/10 text-[#4A3B52]">
            {roleLabel}
          </span>
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-5 mb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-700 text-foreground flex items-center gap-2">
            <User className="w-4 h-4 text-[#4A3B52]" /> Personal Information
          </h2>
          {!editingInfo ? (
            <button
              onClick={() => setEditingInfo(true)}
              className="inline-flex items-center gap-1.5 text-xs font-600 text-[#4A3B52] hover:text-[#3a2e42] transition-colors"
            >
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={saveInfo}
                className="inline-flex items-center gap-1.5 text-xs font-600 text-white bg-[#4A3B52] hover:bg-[#3a2e42] px-3 py-1.5 rounded-lg transition-colors"
              >
          ...
          <div>
            ...
            {editingInfo ? (
              ...
              <div className="flex items-center gap-2 py-2">
                ...
                <p className="text-sm font-500 text-foreground">{displayName}</p>
              </div>
            )}
          </div>
          <div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-5 mb-4">
        <h2 className="font-700 text-foreground flex items-center gap-2 mb-4">
          <Shield className="w-4 h-4 text-[#4A3B52]" /> Account
        </h2>
        <div className="space-y-4">
          <div>
            <label className="text-[11px] uppercase font-600 text-muted-foreground tracking-wide block mb-1">
            ...
            <div className="flex items-center gap-2 py-2">
              <Mail className="w-4 h-4 text-muted-foreground flex-shrink-0" />
              <p className="text-sm font-500 text-foreground font-mono">{user?.email ?? '—'}</p>
            </div>
          </div>
          <div>
            <label className="text-[11px] uppercase font-600 text-muted-foreground tracking-wide block mb-1">
            ...
            <div className="py-2">
              <span className="inline-block text-[11px] font-600 px-2.5 py-0.5 rounded-full bg-[#4A3B52]/10 text-[#4A3B52]">
                {roleLabel}
              </span>
            </div>
          </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-5">
        <h2 className="font-700 text-foreground flex items-center gap-2 mb-4">
          <Lock className="w-4 h-4 text-[#4A3B52]" /> Change Password
        </h2>
        <div className="space-y-3 max-w-sm">
          ...
          <button onClick={savePassword} className="btn-primary px-5 py-2 text-sm mt-2">
            Update Password
          </button>
        </div>
      </div>
